...
observe({
  # Update column names as per changes in the input data
  # Check example is selected or ask to upload a file
  validate(
    need((input$data_file_type=="examplecounts")|((!is.null(input$rdatafile))|(!is.null(input$datafile))), 
         message = "Upload CSV file to start DEG analysis")
  )
...
inputDataReactive <- reactive({
 ...
 # browser()
  ...
  if(input$data_file_type=="examplecounts") {
    # upload example data
    seqdata <- read_csv("data/examplecounts_short.csv") #mousecounts_example.csv")
