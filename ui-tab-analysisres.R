...
tabPanel("Volcano Plots",  
         fluidRow(column(4,wellPanel(
           conditionalPanel("input.analysisres_tabset=='Volcano Plot'",
                            selectizeInput("analysisres_test", label="Select Group Order for Volcano Plot",
                                           choices=NULL),
                            numericInput("analysisres_fold_change_cut",
                                         label="Adjust log2(Fold Change) Threshold\n(based on input FCs)",min= 0, max=20,value=1),
                            numericInput("analysisres_pvalcut",
                                         label="Adjust P-value Threshold",min=0,max=1,value=0.05),
